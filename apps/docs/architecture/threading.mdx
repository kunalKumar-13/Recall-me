...
4. The user can explicitly `POST /v1/threads/{id}/forget` to drop
   the identity row, or wipe the entire cache via Settings →
   *Clear thread cache*.
...
| `POST` | `/v1/threads/{thread_id}/forget` | Drop one thread from the identity cache |
...
- **Replay** (`POST /v1/replay/day`) does not interact with
  threads; it reconstructs the per-day file directly.
