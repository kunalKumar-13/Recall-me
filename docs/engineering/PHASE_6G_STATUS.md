...
Cross-references:
[`docs/product/TRUST.md`](../product/TRUST.md) (the public trust
statement),
...
- Secondary CTA: *Watch demo* (anchors to `#how` until the
  demo video at
  [`DEMO_VIDEO_SCRIPT.md`](../release/DEMO_VIDEO_SCRIPT.md) is
...
the new [`TRUST.md`](../product/TRUST.md). The right-side
...
|---|---|---|
| Hosted demo video | placeholder | The 60-second script lands in [`DEMO_VIDEO_SCRIPT.md`](../release/DEMO_VIDEO_SCRIPT.md); the recording itself is a one-off founder step that needs real machine + browser captures. The *Watch demo* button currently scroll-anchors; once `demo.mp4` exists, `lib/links.ts:demoVideo` flips and every CTA updates. |
| Logo + signing for the Windows installer | external | Gate 7 of [`PUBLIC_ALPHA.md`](../founder/PUBLIC_ALPHA.md). EV cert + signed installer eliminates the SmartScreen prompt; not in 6G's scope (it's a release-pipeline change, not a website change). |
| Verified macOS build | external | Gated by [`MAC_OWNER_NEEDED.md`](../release/MAC_OWNER_NEEDED.md). The Download card for macOS reads *preview* honestly; the page does not pretend otherwise. |
...
[`DOWNLOAD_GUIDE.md`](../release/DOWNLOAD_GUIDE.md), the trust
